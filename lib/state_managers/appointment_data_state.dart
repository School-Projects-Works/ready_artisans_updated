  ...
  void bookAppointment({
    ...
    if (artisan.available) {
      ...
      if (results) {
        await sendMessage(
          artisan.phone,
          'You have a new request from ${client.name}',
        );
...
final appointmentStreamProvider = StreamProvider<List<AppointmentModel>>((
  ref,
) async* {
...
class AppFilter {
  ...
  AppFilter({required this.items, required this.filter});

  AppFilter copyWith({
    List<AppointmentModel>? items,
    List<AppointmentModel>? filter,
  }) {
    return AppFilter(items: items ?? this.items, filter: filter ?? this.filter);
  }
}
  ...
  void filterAppointments(String query) {
    if (query.isNotEmpty) {
      List<AppointmentModel> _filtered = state.items
          .where(
            (element) =>
                element.artisanName.toLowerCase().contains(
                  query.toLowerCase(),
                ) ||
