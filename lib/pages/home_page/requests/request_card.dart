...
class RequestCard extends ConsumerWidget {
    ...
    return Card(
      child: ExpansionTile(
        ...
        children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(
                      data.clientLocation['district'].toString().split(':')[0],
                    ),
                  ],
                ),
                const SizedBox(height: 10),
          ...
          const Divider(color: Colors.black45),
          if (user.id == data.artisanId && data.note.isNotEmpty)
            ListTile(
              contentPadding: EdgeInsets.zero,
              title: Row(
                children: [
              ...
              subtitle: Column(
                children: [
                  const SizedBox(height: 10),
                  Text(data.note, style: normalText(fontSize: 16)),
                ],
              ),
            ),
          if (user.id == data.artisanId && data.status == 'Pending')
            //how accept and reject button
            Row(
              children: [
                Expanded(
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      updateRequest(data: data.copyWith(status: 'Cancelled'));
                    },
                    child: Text(
                      'Cancel Job',
                      style: normalText(color: Colors.white, fontSize: 16),
  ...
  void update(AppointmentModel data) async {
    ...
    await FireStoreServices.updateRequestStatus(data).then((value) async {
      await sendMessage(
        data.clientPhone,
        'Request status updated to ${data.status}',
      );
      CustomDialog.dismiss();
